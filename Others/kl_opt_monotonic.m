...
function [primobj,x,p,samplefnc] = kl_opt_monotonic(n,xmax,C,c_exp,opt_type,r,tol,verbose)
...
while 1
    ...
    grad = getgrad(p,n,r,opt_type); % gradient
    ...
    while 1
        ...
        if min(pnew) > 0 && min(newdvals) > 0 % the p vector better be all positive
            ...
            primobj = getobj(pnew,n,r,opt_type);
...
filename = sprintf('opt_cactus_v%1.2f.mat',C);
save(filename,'p','x','n','r','xmax');

filename = sprintf('cactus_x_v%1.2f.csv',C);
csvwrite(filename,x)
...
filename = sprintf('cactus_cdf_v%1.2f.csv',C);
