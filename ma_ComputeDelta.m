...
if eps <=0
    fprintf("Warning: eps must be greater than zero!")
end
if length(orders)~=length(rdp)
    fprintf("Warning: Inputs must have the same size!")
end
logdeltas = zeros(1,length(rdp));
for i=1:length(rdp)
    if orders(i)<1
        fprintf("Warning: Renyi divergence order must be greater than 1!")
    end
    if rdp(i)<0
        fprintf("Warning: Renyi divergence must be greater than 0!")
    end
